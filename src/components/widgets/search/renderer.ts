...
async function runLinearSearch(animId: string, values: Array<number | string>, target: number | string, status: HTMLElement | null) {
  if (!status) return;
...
async function runBinarySearch(animId: string, values: Array<number | string>, target: number | string, status: HTMLElement | null) {
  if (!status) return;
...
export function renderSearchAnimation(section: any, index: number, lessonId: string) {
  const animId = `search-anim-${lessonId}-${index}`;
  const cells = (section.data || []).map((val: number | string, i: number) => `
