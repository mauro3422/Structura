...
import { renderLabFeedback } from './feedback.ts';
import { renderLaboratoryRulesPanel } from './rulesPanel.ts';
import { renderGraphViewportControls } from '../stage/index.ts';
import { createTableId, resolveLabTables } from './persistence.ts';
import type { InteractiveTableSection, TableExampleSection, TableLaboratorySection } from '../../../core/moduleTypes.ts';
import type { RenderLabTable, TableDefinition } from './types.ts';

export function renderTableExample(section: TableExampleSection, index: number): string {
...
export function renderInteractiveTable(section: InteractiveTableSection, index: number, lessonId: string): string {
...
export function renderTableLaboratory(section: TableLaboratorySection, index: number, lessonId: string): string {
  ...
  return `
    <div class="table-laboratory" id="${labId}" data-lesson-id="${lessonId}">
      ...
      ${renderLabFeedback(labId)}
      ${renderLaboratoryRulesPanel(labId)}
      <div class="lab-observations-panel" id="${labId}-observations" aria-live="polite"></div>
      <div class="graph-stage lab-canvas-wrapper" id="${labId}-stage" data-graph-stage data-viewport-auto-fit="true">
        ${renderGraphViewportControls(labId)}
        <div class="graph-stage__frame lab-canvas-wrapper__frame">
          <div class="graph-stage__surface lab-canvas" id="${labId}-canvas">
...
export function renderLabTable(
  ...
  const tableX = typeof table.x === 'number' ? table.x : 0;
  const tableY = typeof table.y === 'number' ? table.y : 0;
  ...
  const headers = columns
    ...
    <th class="data-table__header-cell" data-pk="${col.isPK || false}" data-fk="${col.isFK || false}" data-col-index="${ci}">
      ...
      ${col.isFK ? '<span class="lab-port lab-port--source" data-graph-port="source" aria-hidden="true"></span>' : ''}
      ${col.isPK ? '<span class="lab-port lab-port--target" data-graph-port="target" aria-hidden="true"></span>' : ''}
  ...
  return renderSectionBlock(
    `
    <div class="lab-table-header" data-stage-drag-handle>
      <button class="lab-table-drag-handle" type="button" data-stage-drag-handle aria-label="Mover tabla">⋮⋮</button>
  ...
  `,
    {
      ...
      marginClass: '',
      animate: false,
      attrs: `data-index="${ti}" data-table-id="${escapeHtml(tableId)}" data-stage-node="true" data-stage-node-id="${escapeHtml(tableId)}" data-stage-x="${tableX}" data-stage-y="${tableY}" style="left:${tableX}px; top:${tableY}px;"`,
    },
  );
}
