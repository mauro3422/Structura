...
import { resolveLabTables } from './state.js';
import { renderSectionBlock } from '../../templates.js';

export function renderTableExample(section, index) {
...
export function renderInteractiveTable(section, index, lessonId) {
  const tableId = `interactive-table-${lessonId}-${index}`;
  const canAddCols = section.canAddColumns !== false;
  const headers = (section.columns || []).map(col => renderInteractiveHeaderCell(col)).join('');
  const addColHeader = canAddCols ? renderAddColumnHeader(tableId, section.columns.length) : '';
  const rows = renderInteractiveRows(section.initialRows || [], section.columns || [], { canAddCols });
  ...
  return renderSectionBlock(`
    ...
    <div class="data-table-wrapper" id="${tableId}" data-table-name="${escapeHtml(section.tableName)}">
      ...
      ${section.canAddRows !== false ? renderAddRowButton(tableId, section.columns || [], section.initialRows?.length || 0, canAddCols) : ''}
    </div>
  `, {
...
export function renderTableLaboratory(section, index, lessonId) {
...
export function renderLabTable(table, ti, lessonId, allTables = []) {
  const tableId = `lab-table-${lessonId}-${ti}`;

  const headers = (table.columns || []).map((col, ci) => `
        ...
        <div class="ref-picker-container">
          <select class="ref-picker" data-action="change-ref" aria-label="Tabla referenciada por ${escapeHtml(col.name)}">
            ...
            ${allTables
              ...
              .map(t => `<option value="${escapeHtml(t.tableName)}" ${col.references === t.tableName ? 'selected' : ''}>${escapeHtml(t.tableName)}</option>`)
              .join('')}
          </select>
        </div>
      ` : ''}
  ...
  const rows = (table.rows || []).map((row, ri) => `
    <tr data-row="${ri}">
      ${row.map((cell, ci) => `
        <td ${ci > 0 || !table.columns[ci]?.autoIncrement ? 'contenteditable="true"' : ''} data-col="${ci}">${escapeHtml(String(cell))}</td>
      `).join('')}
  ...
  return renderSectionBlock(`
    ...
    <div class="data-table-wrapper" id="${tableId}" data-table-name="${escapeHtml(table.tableName)}">
      <table class="data-table">
        <thead><tr id="${tableId}-header">${headers}<th class="data-table__add-col-th"><button class="data-table__add-col" data-table-id="${tableId}" data-col-count="${(table.columns || []).length}">+</button></th></tr></thead>
        <tbody id="${tableId}-body">${rows}</tbody>
      </table>
      <button class="data-table__add-row" data-table-id="${tableId}" data-columns='${JSON.stringify(table.columns)}' data-row-count="${(table.rows || []).length}" data-has-extra-col="true">
        + Agregar fila
      </button>
    </div>
  `, {
